...
func TestBoxScoreTraditionalScraper(t *testing.T) {
	// https://www.wnba.com/game/dal-vs-ind-1022600254/box-score?type=traditional&period=Q1
	if testing.Short() {
	...
	// Every record: sanity-check identity/team fields.
	...
	// One record: validate every field against the real, known Q1-only
	// response captured 2026-08-15 (Lexie Hull, Indiana Fever).
	var hull *model.BoxScoreTraditional
	for i := range records {
	...
	assert.Equal(t, float32(5.12), hull.Minutes)
	assert.Equal(t, int32(0), hull.FieldGoalsMade)
	assert.Equal(t, int32(2), hull.FieldGoalsAttempted)
	assert.Equal(t, float32(0), hull.FieldGoalsPercentage)
	assert.Equal(t, int32(0), hull.ThreePointersMade)
	assert.Equal(t, int32(0), hull.ThreePointersAttempted)
	assert.Equal(t, float32(0), hull.ThreePointersPercentage)
	assert.Equal(t, int32(0), hull.FreeThrowsMade)
	assert.Equal(t, int32(0), hull.FreeThrowsAttempted)
	assert.Equal(t, float32(0), hull.FreeThrowsPercentage)
	assert.Equal(t, int32(0), hull.ReboundsOffensive)
	assert.Equal(t, int32(1), hull.ReboundsDefensive)
	assert.Equal(t, int32(1), hull.ReboundsTotal)
	assert.Equal(t, int32(0), hull.Assists)
	assert.Equal(t, int32(0), hull.Steals)
	assert.Equal(t, int32(0), hull.Blocks)
	assert.Equal(t, int32(0), hull.Turnovers)
	assert.Equal(t, int32(1), hull.FoulsPersonal)
	assert.Equal(t, int32(0), hull.Points)
	assert.Equal(t, int32(-1), hull.PlusMinusPoints)
}
