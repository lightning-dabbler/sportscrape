...
func TestBoxScoreMiscScraper(t *testing.T) {
	// https://www.wnba.com/game/dal-vs-ind-1022600254/box-score?type=misc&period=1stHalf
	if testing.Short() {
	...
	// One record: validate every field against the real, known 1st-half-only
	// response captured 2026-08-15 (Lexie Hull, Indiana Fever).
	var hull *model.BoxScoreMisc
	for i := range records {
	...
	assert.Equal(t, float32(6.68), hull.Minutes)
	assert.Equal(t, int32(0), hull.PointsOffTurnovers)
	assert.Equal(t, int32(0), hull.PointsSecondChance)
	assert.Equal(t, int32(0), hull.PointsFastBreak)
	assert.Equal(t, int32(0), hull.PointsPaint)
	assert.Equal(t, int32(0), hull.OppPointsOffTurnovers)
	assert.Equal(t, int32(4), hull.OppPointsSecondChance)
	assert.Equal(t, int32(2), hull.OppPointsFastBreak)
	assert.Equal(t, int32(4), hull.OppPointsPaint)
	assert.Equal(t, int32(0), hull.Blocks)
	assert.Equal(t, int32(0), hull.BlocksAgainst)
	assert.Equal(t, int32(1), hull.FoulsPersonal)
	assert.Equal(t, int32(0), hull.FoulsDrawn)
}
