...
func TestBoxScoreAdvancedScraper(t *testing.T) {
	// https://www.wnba.com/game/dal-vs-ind-1022600254/box-score?type=advanced&period=All
	if testing.Short() {
	...
	var hull *model.BoxScoreAdvanced
	for i := range records {
	...
	assert.Equal(t, float32(100), hull.EstimatedOffensiveRating)
	assert.Equal(t, float32(100), hull.OffensiveRating)
	assert.Equal(t, float32(104.3), hull.EstimatedDefensiveRating)
	assert.Equal(t, float32(104.3), hull.DefensiveRating)
	assert.Equal(t, float32(-4.3), hull.EstimatedNetRating)
	assert.Equal(t, float32(-4.3), hull.NetRating)
	assert.Equal(t, float32(0), hull.AssistPercentage)
	assert.Equal(t, float32(0), hull.AssistToTurnover)
	assert.Equal(t, float32(0), hull.AssistRatio)
	assert.Equal(t, float32(0), hull.OffensiveReboundPercentage)
	assert.Equal(t, float32(0.167), hull.DefensiveReboundPercentage)
	assert.Equal(t, float32(0.08), hull.ReboundPercentage)
	assert.Equal(t, float32(0), hull.TurnoverRatio)
	assert.Equal(t, float32(0), hull.EffectiveFieldGoalPercentage)
	assert.Equal(t, float32(0), hull.TrueShootingPercentage)
	assert.Equal(t, float32(0.069), hull.UsagePercentage)
	assert.Equal(t, float32(0.069), hull.EstimatedUsagePercentage)
	assert.Equal(t, float32(90.93), hull.EstimatedPace)
	assert.Equal(t, float32(90.93), hull.Pace)
	assert.Equal(t, float32(75.78), hull.PacePer40)
	assert.Equal(t, int32(24), hull.Possessions)
	assert.Equal(t, float32(-0.095), hull.PIE)
}
