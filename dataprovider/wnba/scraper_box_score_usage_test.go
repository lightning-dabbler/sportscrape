...
func TestBoxScoreUsageScraper(t *testing.T) {
	// https://www.wnba.com/game/dal-vs-ind-1022600254/box-score?type=usage&period=Q4
	if testing.Short() {
	...
	// One record: validate every field against the real, known Q4-only
	// response captured 2026-08-15 (Lexie Hull, Indiana Fever).
	var hull *model.BoxScoreUsage
	for i := range records {
	...
	assert.Equal(t, float32(2.78), hull.Minutes)
	assert.Equal(t, float32(0), hull.UsagePercentage)
	assert.Equal(t, float32(0), hull.PercentageFieldGoalsMade)
	assert.Equal(t, float32(0), hull.PercentageFieldGoalsAttempted)
	assert.Equal(t, float32(0), hull.PercentageThreePointersMade)
	assert.Equal(t, float32(0), hull.PercentageThreePointersAttempted)
	assert.Equal(t, float32(0), hull.PercentageFreeThrowsMade)
	assert.Equal(t, float32(0), hull.PercentageFreeThrowsAttempted)
	assert.Equal(t, float32(0), hull.PercentageReboundsOffensive)
	assert.Equal(t, float32(1), hull.PercentageReboundsDefensive)
	assert.Equal(t, float32(0.333), hull.PercentageReboundsTotal)
	assert.Equal(t, float32(0), hull.PercentageAssists)
	assert.Equal(t, float32(0), hull.PercentageTurnovers)
	assert.Equal(t, float32(0), hull.PercentageSteals)
	assert.Equal(t, float32(0), hull.PercentageBlocks)
	assert.Equal(t, float32(0), hull.PercentageBlocksAllowed)
	assert.Equal(t, float32(0), hull.PercentagePersonalFouls)
	assert.Equal(t, float32(0), hull.PercentagePersonalFoulsDrawn)
	assert.Equal(t, float32(0), hull.PercentagePoints)
}
