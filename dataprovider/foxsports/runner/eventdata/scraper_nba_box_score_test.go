...
func TestNBABoxScoreScraper(t *testing.T) {
	...
	for _, statline := range boxScoreStats {
		...
		if s.Player == "Keon Ellis" {
			...
			assert.Equal(t, 37, s.MinutesPlayed, "MinutesPlayed")
			assert.Equal(t, 1, s.FieldGoalsMade, "FieldGoalsMade")
			assert.Equal(t, 2, s.FieldGoalAttempts, "FieldGoalAttempts")
			assert.Equal(t, 0, s.ThreePointsMade, "ThreePointsMade")
			assert.Equal(t, 1, s.ThreePointAttempts, "ThreePointAttempts")
			assert.Equal(t, 0, s.FreeThrowsMade, "FreeThrowsMade")
			assert.Equal(t, 0, s.FreeThrowAttempts, "FreeThrowAttempts")
			assert.Equal(t, 2, s.OffensiveRebounds, "OffensiveRebounds")
			assert.Equal(t, 4, s.DefensiveRebounds, "DefensiveRebounds")
			assert.Equal(t, 6, s.TotalRebounds, "TotalRebounds")
			assert.Equal(t, 0, s.Assists, "Assists")
			assert.Equal(t, 3, s.Steals, "Steals")
			assert.Equal(t, 0, s.Blocks, "Blocks")
			assert.Equal(t, 0, s.Turnovers, "Turnovers")
			assert.Equal(t, 3, s.PersonalFouls, "PersonalFouls")
			assert.Equal(t, 2, s.Points, "Points")
		}
	}
