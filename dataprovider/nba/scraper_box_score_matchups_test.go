...
func TestBoxScoreMatchupsScraper(t *testing.T) {
	...
	for _, s := range records {
		if s.PlayerName == "Shai Gilgeous-Alexander" && s.OpponentPlayerName == "Tyrese Haliburton" {
			...
			assert.Equal(t, float32(0.68), s.MatchupMinutes)
			assert.Equal(t, float32(41), s.MatchupMinutesSort)
			assert.Equal(t, float32(5), s.PartialPossessions)
			assert.Equal(t, float32(0.047), s.PercentageDefenderTotalTime)
			assert.Equal(t, float32(0.047), s.PercentageOffensiveTotalTime)
			assert.Equal(t, float32(0.052), s.PercentageTotalTimeBothOn)
			assert.Equal(t, int32(0), s.SwitchesOn)
			assert.Equal(t, int32(11), s.PlayerPoints)
			assert.Equal(t, int32(13), s.TeamPoints)
			assert.Equal(t, int32(1), s.MatchupAssists)
			...
			assert.Equal(t, int32(3), s.MatchupFieldGoalsMade)
